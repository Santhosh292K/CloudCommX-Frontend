...
export default function ShipmentsPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const approvedOrders = (o.orders ?? []).filter(
  ...
  return (
    <div className="animate-in">
      {/* Live RPC / MOC counters from System 2 analytics */}
      <div className="stats-grid mb-6">
        <div className="card card-sm">
          ...
          <div className="card-sub">
            ...
            {metrics?.rpc_error_count ?? 0} errors
          </div>
        </div>
        <div className="card card-sm">
          ...
          <div className="card-sub">
            System 2 only · DLQ: {metrics?.moc_dlq_count ?? 0}
          </div>
        </div>
        <div className="card card-sm">
          <div className="card-title">Active Shipments</div>
          <div className="card-value">
            {metrics?.total_shipments ?? shipments.length}
          </div>
          <div className="card-sub">In system</div>
        </div>
      </div>
      <div
        className="grid-2 gap-6 mb-6"
      ...
      >
        ...
        <div className="card">
          <div className="section-header">
            <div>
              ...
              <div className="section-subtitle">
                RPC approval → MOC ShipmentDispatched event
              </div>
            </div>
          ...
          <div className="alert alert-info mb-4" style={{ fontSize: 12 }}>
            <strong>RPC Flow:</strong> Calls Warehouse to deduct stock
            (blocking) → then async publishes ShipmentDispatched via MOC
          </div>
          ...
          <form onSubmit={handleDispatch}>
              ...
              >
                ...
                {orders.map((o: any) => (
                  <option key={o.id} value={o.id}>
                    {o.customer_name} — {o.product} × {o.quantity}
                  </option>
                ))}
              </select>
            </div>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  Adds 1.5s delay to MOC event publish
                </div>
              </div>
              <label
                style={{
              ...
              >
                <input
                  type="checkbox"
                  checked={form.eventual_consistency}
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      eventual_consistency: e.target.checked,
                    }))
                  }
                  style={{
                    appearance: "none",
                    width: 36,
                    height: 20,
                    background: form.eventual_consistency
                      ? "var(--accent)"
                      : "var(--border)",
                    borderRadius: 10,
                    cursor: "pointer",
                ...
                <span
                  style={{
                    fontSize: 12,
                    color: form.eventual_consistency
                      ? "var(--accent)"
                      : "var(--text-muted)",
                  }}
                >
            ...
            <button
              ...
              type="submit"
              disabled={loading || !form.order_id}
            >
              {loading ? (
                <>
                  <span
          ...
          {result && !error && (
            <div className="alert alert-success mt-4" style={{ fontSize: 12 }}>
              <CheckCircle size={12} /> Shipment dispatched! RPC approved stock
              deduction.
              {result.eventual_consistency && (
                <>
                  <br />
                  <Clock size={10} /> Eventual consistency ON — event delayed{" "}
                  {result.event_delay_ms}ms
                </>
              )}
            </div>
          )}
          ...
          {selectedShipment ? (
            <>
              ...
              >
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>
                    {selectedShipment.product} × {selectedShipment.quantity}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                    For {selectedShipment.customer_name}
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <span
                    className={`badge ${statusColors[selectedShipment.status] ?? "badge-blue"}`}
                  >
              ...
              <div className="timeline">
                {(selectedShipment.tracking_events ?? []).map(
                  (e: any, i: number) => (
                    <div className="timeline-item" key={i}>
                      <div
                        className={`timeline-dot ${e.event.includes("Delivered") ? "delivered" : ""}`}
                      />
              ...
              {selectedShipment.status !== "delivered" && (
                <button
                  ...
                  onClick={() => {
                    handleDeliver(selectedShipment.id);
      ...
      <div className="card">
        ...
        {shipments.length === 0 ? (
          ...
          >
            No shipments yet. Dispatch one above.
          </div>
        ) : (
              ...
              <tbody>
                {shipments.map((s: any) => (
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`badge ${statusColors[s.status] ?? "badge-blue"}`}
                      >
                        {s.status}
                      </span>
                    </td>
                    <td>
                      {s.eventual_consistency_delay_ms > 0 ? (
                        <span className="badge badge-purple">
                          Eventual ({s.eventual_consistency_delay_ms}ms delay)
                        </span>
                      ) : (
                    ...
                    <td style={{ fontSize: 11 }}>
                      {new Date(s.dispatched_at).toLocaleTimeString()}
                    </td>
