...
export default function AnalyticsPage() {
  ...
  const reliabilityData = [
    {
      ...
      RPC: Math.max(
        0,
        100 -
          ((metrics?.rpc_error_count ?? 0) /
            Math.max(1, metrics?.rpc_call_count ?? 1)) *
            100,
      ),
      MOC: Math.max(
        0,
        100 -
          ((metrics?.moc_dlq_count ?? 0) /
            Math.max(1, metrics?.moc_event_count ?? 1)) *
            100,
      ),
    },
  ...
  const loadChartData = LOAD_LEVELS.map((level) => {
    const rpcRes = loadResults
      .filter((r) => r.load_level === level && r.protocol === "RPC")
      .slice(-1)[0];
    const mocRes = loadResults
      .filter((r) => r.load_level === level && r.protocol === "MOC")
      .slice(-1)[0];
    return {
      load: `${level} reqs`,
    ...
    <div className="animate-in">
      ...
      <div className="stats-grid mb-6">
        <StatCard
          ...
          sub={`${metrics?.rpc_error_count ?? 0} errors`}
          color="var(--accent)"
        />
        <StatCard
          label="RPC Avg Latency"
          value={`${(metrics?.rpc_avg_latency_ms ?? 0).toFixed(1)}ms`}
          sub="Blocking synchronous"
          color="var(--accent)"
        />
        <StatCard
          label="MOC Events"
          value={metrics?.moc_event_count ?? "—"}
          sub={`System 2 only · ${metrics?.moc_dlq_count ?? 0} in DLQ`}
          color="var(--accent-3)"
        />
        <StatCard
          label="MOC Avg Latency"
          value={`${(metrics?.moc_avg_latency_ms ?? 0).toFixed(1)}ms`}
          sub="Publish latency only"
          color="var(--accent-3)"
        />
        <StatCard
          label="Queue Depth"
          value={metrics?.moc_queue_depth ?? 0}
          sub="Current MOC backlog"
          color="var(--orange)"
        />
      </div>
      ...
      <div className="grid-2 gap-6 mb-6">
        ...
        <div className="card">
          ...
          <div className="section-subtitle mb-4">
            RPC (blocking) vs MOC (publish-only)
          </div>
        ...
        <div className="card">
          ...
          <div className="section-subtitle mb-4">
            Comparative radar chart (higher = better)
          </div>
          <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="card mb-6">
        <div className="section-header">
          <div>
            ...
            <div className="section-subtitle">
              Run concurrent requests and compare RPC vs MOC performance
            </div>
          </div>
        ...
        {loadChartData.length > 0 ? (
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={loadChartData}>
                  ...
                  <Legend wrapperStyle={{ fontSize: 12 }} />
                  <Line
                    dataKey="RPC Throughput"
                    stroke="#6378ff"
                    strokeWidth={2}
                    dot={{ r: 4, fill: "#6378ff" }}
                  />
                  <Line
                    dataKey="MOC Throughput"
                    stroke="#00e5ff"
                    strokeWidth={2}
                    dot={{ r: 4, fill: "#00e5ff" }}
                  />
          ...
          >
            Run a load test above to see latency and throughput comparison
            charts.
          </div>
        )}
                ...
                {loadResults
                  ...
                  .map((r: any, i: number) => (
                    <tr key={i}>
                      <td>
                        <span
                          className={`badge ${r.protocol === "RPC" ? "badge-blue" : "badge-cyan"}`}
                        >
                          {r.protocol}
                        </span>
                      </td>
      ...
      <div className="card">
        ...
        <div className="protocol-compare-grid">
          <div className="protocol-box protocol-box-rpc">
            <div className="protocol-title protocol-rpc-title">
              🔵 RPC — CP System
            </div>
            <div className="protocol-prop">
              <span className="protocol-prop-label">Consistency</span>
              <span className="protocol-prop-value badge badge-green">
                ✓ Strong
              </span>
            </div>
            <div className="protocol-prop">
              <span className="protocol-prop-label">Partition Tolerance</span>
              <span className="protocol-prop-value badge badge-green">
                ✓ Yes
              </span>
            </div>
            <div className="protocol-prop">
              <span className="protocol-prop-label">Availability</span>
              <span className="protocol-prop-value badge badge-red">
                ✗ Sacrificed
              </span>
            </div>
            <div
            ...
            >
              On warehouse crash, RPC callers receive immediate errors. The
              system prefers <em>consistency</em> — never serves stale data, but
              loses availability when a node fails.
            </div>
          ...
          <div className="protocol-box protocol-box-moc">
            <div className="protocol-title protocol-moc-title">
              🔷 MOC — AP System
            </div>
            <div className="protocol-prop">
              <span className="protocol-prop-label">Availability</span>
            ...
            <div className="protocol-prop">
              <span className="protocol-prop-label">Consistency</span>
              <span className="protocol-prop-value badge badge-orange">
                ~ Eventual
              </span>
            </div>
            <div
            ...
            >
              On warehouse crash, MOC events queue up and are delivered on
              recovery. The system prefers <em>availability</em> — always
              accepts new events, but consumers may process them late.
            </div>
            <div className="pill-row">
        ...
        >
          ...
          CAP properties. RPC (synchronous) enforces strong consistency at the
          cost of availability — when the warehouse is down, orders fail
          immediately. MOC (asynchronous) keeps the system available and
          partition-tolerant at the cost of eventual consistency — orders are
          placed optimistically and the warehouse syncs later.
...
function StatCard({ label, value, sub, color }: any) {
  return (
    <div className="card card-sm">
      ...
      <div className="card-value" style={{ fontSize: 26, color }}>
        {value}
      </div>
