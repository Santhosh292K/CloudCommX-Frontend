...
import { createOrder, listOrders, getMetrics } from "@/lib/api";
...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const [data, m] = await Promise.all([listOrders(), getMetrics()]);
      setOrders(
        (data.orders ?? []).sort(
          (a: any, b: any) =>
      ...
      >
        ...
        <div className="card">
          <div className="section-header">
            <div>
              ...
              <div className="section-subtitle">
                Triggers synchronous RPC stock check (blocking)
              </div>
            </div>
          ...
          <div className="alert alert-info mb-4" style={{ fontSize: 12 }}>
            <strong>RPC Flow:</strong> Order → <em>blocks</em> until Warehouse
            confirms stock → MOC publishes OrderCreated event (async)
          </div>
          ...
          {rpcBlocking && (
            <div className="alert alert-warning mt-4" style={{ fontSize: 12 }}>
              <Clock size={12} /> <strong>RPC in progress</strong> — caller is
              blocked waiting for warehouse response…
            </div>
          )}
          ...
          {lastResult && !error && (
            <div className="alert alert-success mt-4" style={{ fontSize: 12 }}>
              <CheckCircle size={12} /> Order{" "}
              <strong>{lastResult.order?.id?.substring(0, 8)}…</strong>{" "}
              {lastResult.order?.status}
              <br />
              RPC latency: <strong>{lastResult.rpc_latency_ms}ms</strong>{" "}
              (blocking) | Client roundtrip:{" "}
              <strong>{lastResult.clientLatency}ms</strong>
              <br />
              Stock check: {lastResult.stock_check?.message}
            </div>
          )}
          {error && (
              ...
              <tbody>
                {orders.map((o: any) => (
                  <tr key={o.id}>
                    ...
                    <td>
                      <span
                        className={`badge ${statusColors[o.status] ?? "badge-blue"}`}
                      >
                        {o.status}
                      </span>
                    </td>
                    ...
                    <td>
                      <span
                        className={`badge ${o.event_published ? "badge-green" : "badge-red"}`}
                      >
